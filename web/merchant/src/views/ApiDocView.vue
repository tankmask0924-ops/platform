...
const endpoints: Endpoint[] = [
  ...
  },
  {
    ...
    method: 'GET',
    path: '/products',
    desc: '返回在售的话费商品、售价和按你当前等级计算的每单返佣。',
    params: [{ name: 'business_line', type: 'string', required: true, desc: '目前只支持 recharge' }],
    response: [
      { name: 'id', type: 'int', desc: '商品 ID，下单时作为 product_id' },
      { name: 'name', type: 'string', desc: '商品名称' },
      { name: 'operator', type: 'string', desc: '运营商：mobile 移动，unicom 联通，telecom 电信' },
      { name: 'province', type: 'string|null', desc: '限定省份，null 表示全国' },
      { name: 'charge_speed', type: 'string|null', desc: '到账速度：fast 快充，slow 慢充' },
    ...
    notes: ['卡券商品暂不提供列表接口，商品 ID 可在「商品价格」页查看。'],
  },
  {
    title: '话费下单',
    method: 'POST',

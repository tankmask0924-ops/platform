...
「决定不调用」。**欠款暂停下单（requirements.md 4.5"负余额"，本次"负余额暂停下单"
任务补上，详见第 1 节"商户余额冻结/扣款/解冻/返佣结算"行的 `persistBalance()`/
`debt_since` 说明）**：`place()` 在幂等重放查找（`findByMerchantOrderNoForMerchant()`）
*之后*、校验商品/建单/冻结*之前*检查 `$merchant->debt_since !== null`，命中则直接
...
再试"这两种不同的处理方式。**路由**：按 `supplier_products.priority` 升序迭代，只路由到
